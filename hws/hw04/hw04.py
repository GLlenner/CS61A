...
def balanced(m):
    ...
    if is_mobile(m):
        left_arm = left(m)
        right_arm = right(m)
        left_thing = end(left_arm)
        right_thing = end(right_arm)
        is_balanced = (total_mass(left_thing) * length(left_arm) == total_mass(right_thing) * length(right_arm))

        if is_balanced:
            return balanced(left_thing) and balanced(right_thing)
        else:
            return False
    else:
        return True
...
def max_path_sum(t):
    ...
    rs = 0
    for brance in branches(t):
        if max_path_sum(brance) > rs:
            rs = max_path_sum(brance)
    return rs + label(t)
