...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    select_paragrphs = [x for x in paragraphs if select(x) == True]
    if k >= len(select_paragrphs):
        return ''
    else:
        return select_paragrphs[k]
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        elif cache[(typed,source)][1] >= limit:
            value = cache[(typed,source)][0]
        else:
            cache[(typed,source)] = (value,limit)
        return value
        # END PROBLEM EC
...
def minimum_mewtations(typed, source, limit):
    ...
    dp = []
    #初始化
    for _ in range(0,len(typed) + 1):
        dp.append([0 for x in range(0,len(source)+1)])
    ...
    #动态规划
    for x in range(0,len(typed) + 1):
        for y in range(0,len(source)+1):
            if x == 0 and y == 0:
                dp[x][y] = 0
            elif x == 0:
                dp[x][y] = y
                ...
                dp[x][y] = x
            elif typed[x - 1] == source[y - 1]:
                dp[x][y] = dp[x-1][y-1]
            else:
                dp[x][y] = min(dp[x-1][y],dp[x][y-1],dp[x-1][y-1]) + 1
    
    return dp[len(typed)][len(source)]
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for j in range(0,len(tpp)):
        time = []
        for i in range(1,len(tpp[0])):
            time.append(tpp[j][i] - tpp[j][i-1])
        times.append(time)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word in word_indices:
        ...
        for player in player_indices:
            if times[player][word] < times[min_t][word]:
                min_t = player
        rs[min_t].append(words[word])
    print(rs) 
    # END PROBLEM 10
