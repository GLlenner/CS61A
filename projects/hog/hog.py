...
def boar_brawl(player_score, opponent_score):
    ...
    if abs(player_score  - opponent_score) * 3 > 1:
        return abs(player_score  - opponent_score) * 3
    return 1
...
def always_roll(n):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 6
    def fun(play_score,oppenten_score):
        return n
    return fun
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # END PROBLEM 7
    flag = strategy(0,0)
    for play_score in range(0,goal):
        for oppend_score in range(0,goal):
            if strategy(play_score,oppend_score) != flag:
                return False
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1,11):
        if average(i,dice) > average(result,dice):
            result = i
    return result 
